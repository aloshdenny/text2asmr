...
def kick(host, port):
    files = ["scripts/prep_clap_v2.py", "scripts/train_clap_v3.py", "scripts/pseudo_label_clap.py", "scripts/selftrain_clap.sh"]
...
{writes}
chmod +x /workspace/t2a/scripts/selftrain_clap.sh
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader; nproc
setsid nohup /workspace/t2a/scripts/selftrain_clap.sh > /workspace/selftrain.log 2>&1 < /dev/null &
sleep 3; pgrep -af selftrain_clap | grep -v pgrep | wc -l
echo KICK_DONE
"""
