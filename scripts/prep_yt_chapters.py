...
def main():
    ...
    gm = GpuMel(); w = ShardWriter(a.out, 4000); per_class = Counter(); rng = np.random.default_rng(0)
    def one(v):
        local = None
        try:
            for i in range(4):
            ...
            for c in v["chapters"]:
                ...
                for k in idx:
                    ...
                    out.append((repeatpad(seg), {"uid": f"yt:{v['id']}:{c['start'] + k * a.hop:.1f}", "label": c["cls"], "text": TEXTS.get(c["cls"], []), "split": split, "source": f"yt:{v['id']}", "chapter": c["title"], "rms_db": round(float(db), 1)}))
            return v["id"], out
