...
PREFS = os.environ.get("T2A_PREFS", "RTX 6000 Ada,L40S,L40,A40,RTX A6000,RTX 4090,RTX A5000,RTX 5090").split(",")
SUBSET = os.environ.get("T2A_SUBSET", "v2/train_subset_balanced.jsonl")
TRAIN_ARGS = os.environ.get("T2A_TRAIN_ARGS", "--batch 256 --epochs 4 --lr 2e-5 --text-lr 2e-5 --freeze-text-frac 0 --warmup 200 --eval-every 500 --wait-for-prep --min-rows 60000 --total-rows 246000")
DISK = os.environ.get("T2A_DISK", "120")
...
def create(key, token, pub, gid, name, cloud="SECURE"):
    env = ", ".join(f'{{key:{json.dumps(k)},value:{json.dumps(v)}}}' for k, v in {"HF_TOKEN": token, "PUBLIC_KEY": pub}.items())
    m = f'''mutation {{ podFindAndDeployOnDemand(input: {{ cloudType: {cloud} gpuCount: 1 volumeInGb: 0 containerDiskInGb: {DISK}
      minVcpuCount: 8 minMemoryInGb: 30 gpuTypeId: "{gid}" name: "t2a-clap-v2" imageName: "{IMAGE}" dockerArgs: "" ports: "22/tcp"
...
export PYTHONUNBUFFERED=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True; export DEBIAN_FRONTEND=noninteractive
...
p = hf_hub_download("aoxo/clap-ft-data", "{SUBSET}", repo_type="dataset", token=os.environ["HF_TOKEN"])
shutil.copy(p, "label_tool/train_subset.jsonl"); print("subset_lines", sum(1 for _ in open("label_tool/train_subset.jsonl")))
PY
nohup python scripts/prep_clap_v2.py --subset label_tool/train_subset.jsonl --out /workspace/mel --workers 24 > /workspace/prep.log 2>&1 &
nohup python scripts/train_clap_v2.py --data /workspace/mel --out /workspace/clap-v2 {TRAIN_ARGS} --push-to {PUSH_TO} > /workspace/train.log 2>&1 &
...
def main():
    ...
    for cloud in ("COMMUNITY", "SECURE"):
        for name in PREFS:
            try:
                pod = create(key, token, pub, gpu_id(key, name), name, cloud); print("created", cloud, name, json.dumps(pod)); break
            except Exception as e: print(f"{cloud} {name}: {str(e)[:90]}")
        if pod: break
